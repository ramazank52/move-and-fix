...
- [x] THEME-HOME-PILOT-1 — Değişiklik öncesi checkpoint al; yalnız 01 Ana Sayfa route/wrapper davranışını ve callback bağlarını envanterle
...
- [ ] CLOSURE-2 — Outbox PII/deep-link/idempotency/revoke/IDOR negatif testlerini ve gerçek in-app teslim kanıtını ekle (P17: PRIVATE_STAGING_TIDB_REQUIRED; Blok 1 kapanmadı)
- [ ] CLOSURE-3 — Migration-backed maxConcurrentJobs=1, canonical job lifecycle role matrix ve capability partial deactivation uygulamasını ekle (P17: Blok 1 PASS/onayına kadar başlatma)
...
- [ ] CONTINUE-2 — Outbox worker auto-start/safe shutdown ve offer-accept revoke kaynakta tamam; transactional request enqueue ile cancel/suspend lifecycle revoke bağlantıları PRIVATE_STAGING_TIDB_REQUIRED
- [ ] CONTINUE-3 — Outbox RBAC/IDOR/PII/idempotency/retry/revoke/concurrency gerçek TiDB testlerini tamamla; gerçek push teslimini CODE_READY_NOT_CONFIGURED bırak
...
- [x] P31P33-4 — Review + pending moderation atomic writer testlerini rollback/idempotency/concurrency bakımından genişlet; approved-only rating persistence ve aggregate akışını tamamla
- [x] P31P33-5 — Rating reconciliation apply yolunu MFA/RBAC, dry-run varsayılanı, plan hash/schema drift, batch/checkpoint/resume/idempotency ve PII-minimal audit ile kod-hazır yap
- [x] P31P33-6 — 0090 capacity adayını migration-first sözleşme ile doğrula; maxConcurrentJobs fail-closed default 1, admin-only değişim, atomic winner/capacity guard ve kaynak yarış testlerini uygula
