...
export type ExploreFilterTab = "all" | "emergency" | "km_based";

export type ExploreCategory = { id: number | string; slug: string; name: string; pricingType?: string | null; professionalCount?: number | string | null; color?: string | null };
export type ExploreProvider = { id: number | string; displayName: string; isVerified?: number | boolean | null; rating?: number | null; moveScore?: number | null };
export type ExploreScreenViewProps = {
  categories: readonly ExploreCategory[];
  nearbyProviders: readonly ExploreProvider[];
  categoriesLoading: boolean;
  categoriesError: boolean;
  providersLoading: boolean;
  providersError: boolean;
  initialSearchQuery?: string;
  initialFilter?: ExploreFilterTab;
...
export function ExploreScreenView({ categories, nearbyProviders, categoriesLoading, categoriesError, providersLoading, providersError, initialSearchQuery = "", initialFilter = "all", onOpenCategory, onOpenProvider, onRetryCategories, onRetryProviders }: ExploreScreenViewProps) {
  const colors = useColors();
  const { t } = useTranslation();
  const [searchQuery, setSearchQuery] = useState(initialSearchQuery);
  const [activeFilter, setActiveFilter] = useState<ExploreFilterTab>(initialFilter);

  const categoriesQuery = { data: categories, isLoading: categoriesLoading, error: categoriesError ? new Error("categories") : null, refetch: onRetryCategories };
  const providersQuery = { data: nearbyProviders, isLoading: providersLoading, error: providersError ? new Error("providers") : null, refetch: onRetryProviders };
  ...
  const filterTabs: { id: ExploreFilterTab; label: string; icon: string }[] = [
