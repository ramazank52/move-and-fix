...
CREATE TABLE `job_completion_proofs` (
	...
	`releaseReason` enum('customer_approval','auto_release','admin_resolution'),
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
...
--> statement-breakpoint
CREATE INDEX `completion_disputes_status_created_idx` ON `completion_disputes` (`status`,`createdAt`);--> statement-breakpoint
CREATE INDEX `completion_disputes_proof_idx` ON `completion_disputes` (`completionProofId`);--> statement-breakpoint
CREATE INDEX `escrow_release_events_request_idx` ON `escrow_release_events` (`requestId`);--> statement-breakpoint
CREATE INDEX `job_completion_proofs_due_status_idx` ON `job_completion_proofs` (`status`,`responseDueAt`);--> statement-breakpoint
