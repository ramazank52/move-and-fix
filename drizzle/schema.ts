...
export const marketplaceOpportunityNotifications = mysqlTable(
  "marketplace_opportunity_notifications",
  {
    ...
    status: mysqlEnum("status", ["queued", "processing", "delivered", "revoked", "failed", "dead_letter"]).default("queued").notNull(),
    ...
    deliveryNotificationId: int("deliveryNotificationId"),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
  },
  (table) => [
    ...
    index("marketplace_opportunity_notification_claim_idx").on(table.status, table.nextAttemptAt, table.claimUntil),
  ],
);
