...
| `COMPONENT_FIXTURE` | 01 Ana Sayfa pilot | Runtime component render available | Runtime component render available | Owner must supply iPhone Safari screenshots | WAITING_FOR_OWNER_PHYSICAL_SCREENSHOTS | The production `HomeScreenView` is shared by the real route wrapper and a development-only adapter using immutable in-memory props; normal/loading/empty/disabled states render, but this is not route E2E, auth, DB or native evidence |
