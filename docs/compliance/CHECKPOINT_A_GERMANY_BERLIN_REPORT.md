...
| `pnpm test` | PASS | **127 dosya / 740 test** |
| `pnpm lint` | PASS | Expo lint, 0 çıktı/uyarı |
| `NODE_OPTIONS=--max-old-space-size=512 pnpm exec tsc --noEmit --skipLibCheck` | ENVIRONMENTAL LIMIT | Node heap OOM; tip hatası raporlanmadı |
| `NODE_OPTIONS=--max-old-space-size=1792 pnpm exec tsc --noEmit --skipLibCheck` | PASS | TypeScript error yok |
