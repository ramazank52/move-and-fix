    ...
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 36 }} keyboardShouldPersistTaps="handled">
        <View className="flex-row items-center justify-between">
          <Pressable onPress={() => router.back()} style={({ pressed }) => ({ padding: 8, opacity: pressed ? 0.6 : 1 })} accessibilityLabel="Geri">
        ...
        {isProvider ? (
          <View style={{ marginTop: 16, borderRadius: 18, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, overflow: "hidden" }}>
            <Pressable onPress={() => setExpanded((value) => !value)} style={({ pressed }) => ({ minHeight: 54, paddingHorizontal: 16, flexDirection: "row", alignItems: "center", justifyContent: "space-between", opacity: pressed ? 0.75 : 1 })}>
            ...
            {expanded ? <View style={{ borderTopWidth: 1, borderTopColor: colors.border, padding: 16 }}>
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                {CATEGORIES.map(([value, label]) => <Pressable key={value} onPress={() => setCategory(value)} style={({ pressed }) => ({ borderRadius: 999, paddingHorizontal: 10, paddingVertical: 7, backgroundColor: category === value ? colors.primary : colors.background, opacity: pressed ? 0.7 : 1 })}><Text style={{ color: category === value ? "#fff" : colors.foreground, fontSize: 12, fontWeight: "700" }}>{label}</Text></Pressable>)}
              </View>
              <TextInput value={amount} onChangeText={setAmount} keyboardType="number-pad" placeholder="Tutar (TL)" placeholderTextColor={colors.muted} style={{ marginTop: 14, borderRadius: 12, borderWidth: 1, borderColor: colors.border, color: colors.foreground, minHeight: 48, paddingHorizontal: 12 }} />
              <TextInput value={description} onChangeText={setDescription} placeholder="Masraf açıklaması" placeholderTextColor={colors.muted} multiline style={{ marginTop: 10, borderRadius: 12, borderWidth: 1, borderColor: colors.border, color: colors.foreground, minHeight: 72, padding: 12, textAlignVertical: "top" }} />
              <TextInput value={vendorName} onChangeText={setVendorName} placeholder="Satıcı / mağaza (opsiyonel)" placeholderTextColor={colors.muted} style={{ marginTop: 10, borderRadius: 12, borderWidth: 1, borderColor: colors.border, color: colors.foreground, minHeight: 48, paddingHorizontal: 12 }} />
              <Pressable onPress={pickReceipt} style={({ pressed }) => ({ marginTop: 12, minHeight: 44, borderRadius: 12, alignItems: "center", justifyContent: "center", borderWidth: 1, borderStyle: "dashed", borderColor: colors.primary, opacity: pressed ? 0.7 : 1 })}><Text style={{ color: colors.primary, fontWeight: "800" }}>{receipt ? "Belge seçildi" : "Fiş / belge görseli ekle"}</Text></Pressable>
              <Pressable disabled={createExpense.isPending || uploadMedia.isPending} onPress={saveExpense} style={({ pressed }) => ({ marginTop: 12, minHeight: 48, borderRadius: 12, backgroundColor: colors.primary, alignItems: "center", justifyContent: "center", opacity: pressed || createExpense.isPending || uploadMedia.isPending ? 0.7 : 1 })}><Text className="font-bold text-white">{createExpense.isPending || uploadMedia.isPending ? "Kaydediliyor…" : "Masrafı kaydet"}</Text></Pressable>
            </View> : null}
          </View>
        ) : null}
