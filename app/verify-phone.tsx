  ...
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]} className="px-6">
      <View style={{ flex: 1, justifyContent: "center" }}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel="Telefon doğrulama kodunu onayla" accessibilityState={{ busy: verifyCode.isPending, disabled: !user || !/^\d{6}$/.test(code) || verifyCode.isPending }} disabled={!user || !/^\d{6}$/.test(code) || verifyCode.isPending} onPress={() => verifyCode.mutate({ purpose: "verify_phone", code })} style={({ pressed }) => ({ marginTop: 20, minHeight: 54, borderRadius: 15, alignItems: "center", justifyContent: "center", backgroundColor: user && /^\d{6}$/.test(code) ? colors.primary : colors.muted, opacity: pressed ? 0.85 : 1 })}>{verifyCode.isPending ? <ActivityIndicator color="#fff" /> : <Text style={{ color: "#fff", fontSize: 16, fontWeight: "800" }}>Doğrula</Text>}</Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel="Telefon doğrulama kodunu yeniden gönder" accessibilityState={{ busy: requestCode.isPending, disabled: !user || requestCode.isPending }} disabled={!user || requestCode.isPending} onPress={() => requestCode.mutate({ purpose: "verify_phone" })} style={({ pressed }) => ({ marginTop: 18, alignSelf: "center", opacity: pressed ? 0.65 : 1 })}><Text style={{ color: colors.primary, fontWeight: "700", fontSize: 14 }}>{requestCode.isPending ? "Kod isteniyor..." : "Kodu tekrar gönder"}</Text></Pressable>
      </View>
    </ScreenContainer>
  );
}
