...
| `pnpm test` | PASS | **125 dosya / 732 test** |
...
512 MB TypeScript denetimi bu repo/sandbox için Node heap sınırına çarpmıştır; bu sonucu PASS olarak sunmuyorum. Aynı denetim geniş heap ile tip hatası olmadan tamamlanmıştır. Audit ZIP’i altındaki kopya test kaynaklarının yanlışlıkla test keşfine girmesi düzeltildi: `vitest.config.ts` yalnız gerçek `tests/**/*.{test,spec}.{ts,tsx}` kapsamını içerir. Bu bir test atlama değildir; gerçek test dizinindeki 125 dosyanın tamamı yeniden çalıştırılmıştır.
