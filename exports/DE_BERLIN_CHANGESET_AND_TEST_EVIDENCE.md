...
The detailed role/RPC boundary and the executed negative test command are recorded in `DE_BERLIN_AUTHORIZATION_BOUNDARY_EVIDENCE.md`. TiDB/MySQL RLS is not available in this deployment, so no database RLS test is claimed; the evidence instead records the application-layer server/tRPC boundary and the absence of a new country-scoped write RPC.
...
The 512 MB command must remain an external CI-capacity gate until a measured, approved build-budget decision exists. The final full regression after the legal-locale contract addition is **125 test files / 732 tests PASS**; targeted Germany/Türkiye policy evidence is **4 files / 20 tests PASS**.

## Test Discovery Integrity Correction

The read-only export package intentionally carries copies of selected test files as audit evidence. Vitest’s default repository-wide discovery could treat those incomplete copies as runnable tests. `vitest.config.ts` now explicitly includes every source test under `tests/**/*.{test,spec}.{ts,tsx}` and excludes `exports/**`, `dist/**`, and `node_modules/**`. `tsconfig.json` likewise excludes `exports`, so incomplete audit copies cannot create artificial TypeScript errors. A file-system audit confirmed that the source `tests/` directory contains 125 matching test/spec files, and the subsequent full regression ran all 125. This correction prevents an export artefact from changing test execution; it does not delete, skip, or weaken any project test.
